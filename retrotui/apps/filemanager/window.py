...
from ...utils import safe_addstr, check_unicode_support, theme_attr
...
from .operations import (
    ...
    create_directory, create_file, _is_long_file_operation
)
from .preview import get_preview_lines, get_entry_info_lines, IMAGE_EXTENSIONS
...
class FileManagerWindow(Window):
    ...
    def rename_selected(self, new_name):
        ...
        return perform_move(entry.full_path, dest)

    def copy_selected(self, dest_path):
        entry = self._selected_entry()
        if not entry: return
        return perform_copy(entry.full_path, dest_path)

    def move_selected(self, dest_path):
        entry = self._selected_entry()
        if not entry: return
        return perform_move(entry.full_path, dest_path)

    def _dual_copy_move(self, is_move=False):
    ...
    def set_bookmark(self, slot, path=None):
        ...
        return set_bookmark(self.bookmarks, slot, path)
    ...
    def draw(self, app):
        ...
        border_attr = theme_attr('window.border')
        if self.active:
            border_attr = theme_attr('window.border_active')

        self._draw_border(app, border_attr)
    ...
    def _draw_single_pane(self, app, border_attr):
        ...
        if prev_x:
            ...
            for i in range(bh):
                if i < len(lines):
                     safe_addstr(app.stdscr, by + i, prev_x, lines[i][:prev_w], theme_attr('window.text'))
    ...
    def _draw_pane_contents(self, app, pane_id, x, y, w, h, content, scroll, selected, error_msg, is_active=True):
        ...
        bar_attr = theme_attr('window.title_active' if is_active and self.active else 'window.title_inactive')
        ...
        dir_attr = theme_attr('filemanager.dir')
        safe_addstr(app.stdscr, y + 1, x, sep_line[:w], dir_attr)

        if error_msg:
             safe_addstr(app.stdscr, y + 2, x + 2, f'Error: {error_msg}'[:w-2], theme_attr('window.text'))
             return
        ...
        for k in range(display_h):
             ...
             if is_sel and is_active and self.active:
                 attr = theme_attr('menu.selected')
             elif entry_obj:
                 if entry_obj.is_dir:
                     attr = theme_attr('filemanager.dir')
                     ...
                     try:
                        if os.access(entry_obj.full_path, os.X_OK):
                            attr = theme_attr('filemanager.exe')
                     except: pass
    ...
    def handle_key(self, key):
        ...
        if key == self.KEY_F5:
            if self.dual_pane_enabled:
                 return self._dual_copy_move(is_move=False)
            return AppAction.FM_COPY
        elif key == self.KEY_F4:
            if self.dual_pane_enabled:
                 return self._dual_copy_move(is_move=True)
            return AppAction.FM_MOVE
        elif key == self.KEY_F2: return AppAction.FM_RENAME
        elif key == self.KEY_F6: return AppAction.FM_MOVE
        elif key == self.KEY_F7: return AppAction.FM_NEW_DIR
        elif key == self.KEY_F8: return AppAction.FM_NEW_FILE
        elif key == self.KEY_INSERT: return AppAction.FM_TOGGLE_SELECT

        if isinstance(key, str):
            if key in 'hH': return AppAction.FM_TOGGLE_HIDDEN
            if key in 'dD': return self.toggle_dual_pane() or ActionResult(ActionType.REFRESH)
            if key in 'uU': return AppAction.FM_UNDO_DELETE
            if key == '\x7f' or key == curses.KEY_BACKSPACE:
                self.navigate_parent()
                return ActionResult(ActionType.REFRESH)

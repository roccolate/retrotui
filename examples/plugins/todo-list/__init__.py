...
class Plugin(RetroApp):
    ...
    def draw_content(self, stdscr, x, y, w, h):
        ...
        for i, todo in enumerate(self.todos[:h]):
            if isinstance(todo, dict):
                ...
                pr = str(todo.get('priority', ' '))[:1]
                text = str(todo.get('text', ''))
            else:
                check = '[ ]'
                pr = ' '
                text = str(todo)
            line = f"{pr} {check} {text}"
            a = attr
            if i == self.selected:
    ...
    def handle_key(self, key):
        if key == ord('j') or key == 258:  # down
            self.selected = min(self.selected + 1, max(len(self.todos) - 1, 0))
        elif key == ord('k') or key == 259:  # up
            self.selected = max(self.selected - 1, 0)
        elif key == ord(' '):  # toggle done
            if self.todos:
                todo = self.todos[self.selected]
                if isinstance(todo, dict):
                    todo['done'] = not todo.get('done')
                    self._save()
            ...
            if self.todos:
                todo = self.todos[self.selected]
                if isinstance(todo, dict):
                    todo['text'] = todo.get('text','') + ' (edited)'
                    self._save()

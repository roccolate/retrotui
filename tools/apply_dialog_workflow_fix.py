#!/usr/bin/env python3
"""Apply the typed dialog workflow stabilization patch."""
...
def write_dialog_workflow_module() -> None:
    Path("retrotui/core/dialog_workflow.py").write_text(
        dedent(
            '''\
...
def patch_dispatcher() -> None:
    path = Path("retrotui/core/dialog_dispatch.py")
    text = path.read_text(encoding="utf-8")
    marker = "    def resolve_dialog_result(self, result_idx):\n"
    if text.count(marker) != 1:
        raise SystemExit("dialog_dispatch.py: resolve marker mismatch")
    ...
    path.write_text(prefix + indent(methods, "    "), encoding="utf-8")


def patch_action_runner() -> None:
    replace_once(
        "retrotui/core/action_runner.py",
...
def patch_file_operations() -> None:
    path = "retrotui/core/file_operations.py"
    replace_once(
        path,
...
def write_dialog_tests() -> None:
    Path("tests/test_dialog_dispatch.py").write_text(
        dedent(
            '''\
            import unittest
